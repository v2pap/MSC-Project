...
class HeatMap():
    ...
    def _build_cht_view(self):
        ...
        self.widgets['fig'] = bqp.Figure(marks=[], axes=[self.widgets['ax_x'], self.widgets['ax_y']], padding_y=0.0,
                            layout={'min_width':'800px','min_height':'600px'},fig_margin={'top':20,'right':0,'bottom':130,'left':130})
        self.widgets['heat_map_view'] = VBox([self.widgets['fig'] ], layout={'width':'80%'})
        self.show()
    ...
    def push(self,df,group1,group2,weight_type):
        ...
        grid_df = self.get_grid_map_data_from_df(group1,group2,weight_type)
        self.create_new_grid(grid_df)

    def get_grid_map_data_from_df(self,group1,group2,weight_type):
        totals = self.df.groupby(by=[group1,group2]).sum().unstack().stack(dropna=False).fillna(0)
...
class ThirdPartyRatings(VBox):
    ...
    def __init__(self,ratings_scale=['AAA','AA','A','BBB','BB','B','CCC']):
        self.ratings = ratings_scale
        self.upper = self._build_banner('','')
        self.lower = self._build_rating_buttons('','')
        self.date = HTML()
        super().__init__(children = [self.upper,self.lower,self.date])

    def _build_banner(self,score,color):
        html_label_text = '<p style="color: #F6980A;font-size:30px;line-height: 95px;">3rd Party ESG Rating</p>'
        ...
        html_score_text = '''<html>
            ...
            </html>'''.replace('thescore',score).replace('color',color)
    ...
    def _build_rating_buttons(self,score,color):
        children = [Button(description=rating,disabled=False,layout={'width':'50px'},style={'button_color' : color})  if rating == score else
    ...
    def update(self,score,date):

        self.date.value = '<p style="color: #F6980A;font-size:20px;">Last Updated: {}</p>'.format(date)
        self.upper = self._build_banner(score,'#F6980A')
        self.lower = self._build_rating_buttons(score,'#F6980A')
        self.children = [self.upper,self.lower,self.date]
...
class InternalRatings(VBox):
    ...
    def __init__(self):
        ...
        super().__init__(children = [self.upper],layout={'width':'auto'})


    def _build_banner(self,score):

        html_label_text = '<p style="color: #fffff;font-size:30px;line-height: 85px;">Internal ESG Flag</p>'
        ...
        #green:  #00e600 amber: #ff751a red: #ff1a1a, greay : #808080
        label = HTML(html_label_text)
        score_circle = HTML(html_score_text)
