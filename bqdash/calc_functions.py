...
def verify_user_input(column_mappings,df,colors={},title=''):
    wrong_type_dict = {}

    if not isinstance(column_mappings, dict):
        wrong_type_dict['column_mappings'] = type(column_mappings)

    if not isinstance(df, pd.DataFrame):
        wrong_type_dict['df'] = type(df)

    if not isinstance(colors, dict):
        wrong_type_dict['colors'] = type(colors)

    if not isinstance(title, str):
        wrong_type_dict['title'] = type(title)

    return wrong_type_dict
